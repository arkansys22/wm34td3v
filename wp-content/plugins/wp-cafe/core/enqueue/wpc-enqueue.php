    ...
    public function wpc_settings_obj() {
        ...
        if ( $settings ) {
            ...
            if( isset( $settings['wpc_early_bookings'] ) && isset( $settings['wpc_early_bookings_value'] ) ){
                $wpc_max_day = date( 'Y-m-d', strtotime( date( 'Y-m-d' ) . "+${wpc_early_bookings_value} ${wpc_early_bookings}" ) );
            }elseif( empty( $settings['wpc_early_bookings_value'] )){
                switch ( $wpc_early_bookings ) {
                    case '1day':
                        $wpc_max_day = date( 'Y-m-d', strtotime( date( 'Y-m-d' ) . "1 day" ) );
                        break;

                    case '1week':
                        $wpc_max_day = date( 'Y-m-d', strtotime( date( 'Y-m-d' ) . "1 week" ) );
                        break;

                    case '1month':
                        $wpc_max_day = date( 'Y-m-d', strtotime( date( 'Y-m-d' )) );
                        break;
